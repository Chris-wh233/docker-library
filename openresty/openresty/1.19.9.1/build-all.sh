...
install_from_source() {
    ...
    cd /tmp
    echo "curl -fSL '$URL' -o '$SOURCE_FILE'"
    ...
    if [ "$BUILD_ARCHITECTURE" = "loongarch64" ]; then
	    ...
	    autoreconf -fiv
            ...
            #curl -fSL --retry 5 --retry-delay 2 --connect-timeout 10 --max-time 60 'https://git.savannah.gnu.org/gitweb/?p=config.git;a=blob_plain;f=config.guess;hb=HEAD' -o config.guess
            #curl -fSL --retry 5 --retry-delay 2 --connect-timeout 10 --max-time 60 'https://git.savannah.gnu.org/gitweb/?p=config.git;a=blob_plain;f=config.sub;hb=HEAD' -o config.sub
        fi

        if echo "$SOURCE_DIR" | grep -q "openresty"; then
            echo 'patching LuaJIT for LoongArch64'
            #LuaJIT_VERSION=v2.1-20210510
